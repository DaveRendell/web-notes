...
import { EllipsisVertical, Image, PanelLeftOpen } from 'lucide-react';
import { useEffect, useRef, useState } from 'react';
import { AnimatedPopover } from './AnimatedPopover';
import { ImageActionItems } from './ImageActionItems';
import { formatImageSize } from '../lib/vaultImages';

export function ImageViewer({ onOpenSidebar }: { onOpenSidebar?: () => void } = {}) {
  ...
  return <main className="viewer image-viewer">
    <header className="image-viewer-header">
      {onOpenSidebar && <button className="icon-button" type="button" onClick={onOpenSidebar} aria-label="Show file sidebar" title="Show file sidebar"><PanelLeftOpen size={18} /></button>}
      ...
      <div className="note-actions-menu" ref={menu}>
        ...
        <AnimatedPopover className="header-menu-popover" role="menu" isOpen={open} onEscape={() => setOpen(false)}><ImageActionItems image={selectedFile} onClose={() => { setOpen(false); trigger.current?.focus(); }} /></AnimatedPopover>
      </div>
    </header>
